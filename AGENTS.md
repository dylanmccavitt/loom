...
- **Factory envelope bootstrap (per-VM):** the Factory Nucleus envelope is VM-local state (`~/.loom/factory-nucleus/<id>/envelope/envelope.yaml`) and is never committed. On a fresh VM, re-create and bind it with: `npm run factory -- init-envelope --root .` then `npm run factory -- bind-tracker --root . --provider linear --team Loom --project "Workflow Nucleus"`. The tracked `.loom.yml` pointer carries identity only (`factory: loom`); policy stays in the local envelope.
