...
3. Run `node scripts/retro-packet.mjs --pr <merged-pr-number>` from the repo after merge. The generator creates a pending retro packet branch/worktree, writes decision-log, exemplar, rule, and coverage-gap candidates under `retro/pr-{number}/`, and prints the exact `gh pr create` command. Use `--pr-create` only when intentionally opening that retro PR.
