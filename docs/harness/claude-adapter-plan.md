...
- Project-specific workflow policy belongs in a repo `CLAUDE.md` bridge plus repo-local `nucleus/skills` where it is useful across harnesses.
...
Claude reads `CLAUDE.md`, not `AGENTS.md`, so the instruction bridge imports `@AGENTS.md` when the repo has shared workflow guidance. Skills are populated later through curated per-skill symlinks from shared `nucleus/skills` roots, never by whole-root symlinks or copies.
...
| --- | --- | --- | --- |
| `btw` | `omp-btw` | `nucleus/skills/omp-btw` | future reviewed symlink candidate |
| `guided-goal` | `omp-guided-goal` | `nucleus/skills/omp-guided-goal` | future reviewed symlink candidate |
| `handoff` | `omp-handoff` | `nucleus/skills/omp-handoff` | dry-run `templates/skills/omp-handoff/SKILL.md` example |
| `omfg` | `omp-complaint-to-rule` | `nucleus/skills/omp-complaint-to-rule` | future reviewed symlink candidate |
| `plan` | `omp-plan` | `nucleus/skills/omp-plan` | dry-run `templates/skills/omp-plan/SKILL.md` example |
| `tan` | `omp-tangent` | `nucleus/skills/omp-tangent` | future reviewed symlink candidate |
...
| --- | --- | --- | --- |
| `~/.agents/skills` | user shared | Reusable workflow skills can drift from repo `nucleus/skills`. | document-only; prefer curated per-skill symlink candidates; forbid-bulk-symlink |
...
| `repo:nucleus/skills` | repo project | Project skills can collide by name across scopes. | document-only; keep as project-scoped source of truth |
...
9. Require a separate future issue and PR before writing to live `~/.claude`, project `.claude`, or shared `nucleus/skills`.
...
| Which skill candidates should be repo-local nucleus/skills versus personal ~/.agents/skills, and how should Claude consume them? | Use the workflow-kit split: repo workflow and domain skills go in `nucleus/skills`; general reusable workflow skills go in `~/.agents/skills`; Claude consumes them via curated per-skill symlinks or reviewed adapter templates, never whole-root copies. |
