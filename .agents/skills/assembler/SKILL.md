...
Home: `.agents/envelope/` in the target repo. Four bindings:
...
This envelope is the **single binding point**. Every kit skill reads it instead of hardcoding a tracker. See [ENVELOPE.md](ENVELOPE.md) for the full shape of each binding.
...
4. **Verify.** Cross-check the bindings agree and the envelope is complete before declaring the repo ready; see [VERIFY.md](VERIFY.md). A refresh proposes a diff and asks before overwriting.
...
- **Single binding point.** The envelope is the one place repo facts live; skills read it, they do not re-derive it.
