...
1. Add the `openaiDeveloperDocs` MCP server yourself using your agent's MCP configuration (streamable HTTP transport, URL `https://developers.openai.com/mcp`).
2. If it fails due to permissions/sandboxing, immediately retry with escalated permissions and include a 1-sentence justification for approval. Do not ask the user to do it yet.
3. Only if the escalated attempt fails, ask the user to add the MCP server.
4. Ask the user to restart the agent.
