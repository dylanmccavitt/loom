...
Issue #40 classifies every built-in OMP slash command indexed by the #39 snapshot at `distributions/snapshots/omp-builtins/commands.json`. The canonical machine-readable matrix is `portability-matrix.json`; this file summarizes the policy and review points before any adapter or skill ports are implemented.
